...
contract SablierTest is Test {
    ...
    function testCreateVesting() public {
        LockupLinear.CreateWithRange memory params = _getInputForFullVest();

        vm.prank(owner);
        sablierV2LockupLinear.createWithRange(params);

        assertEq(address(sablierV2LockupLinear.getAsset(streamId)), address(token));
        assertEq(sablierV2LockupLinear.getStartTime(streamId), block.timestamp + 1 weeks);
        ...
        assertEq(uint256(sablierV2LockupLinear.statusOf(streamId)), 0); // will start in a week, so status is pending
        assertEq(token.balanceOf(owner), 0);
        assertEq(token.balanceOf(recipient), 0);

        vm.warp(block.timestamp + 1 weeks);
        assertEq(uint256(sablierV2LockupLinear.statusOf(streamId)), 1); // it has started so status is pending
    }
    ...
    function _getInputForFullVest() internal view returns(LockupLinear.CreateWithRange memory params) {
        return params = LockupLinear.CreateWithRange({
            sender: owner,
            recipient: recipient,
            ...
            range: LockupLinear.Range({
                start: uint40(block.timestamp + 1 weeks), // timestamp at which vesting starts
                cliff: uint40(block.timestamp + 4 weeks), // timestamp at which cliff ends i.e 4 weeks after start time
                end: uint40(block.timestamp + 52 weeks) // timestamp at which vesting ends - inclusive of cliff, so add cliff time
            }),
            broker: Broker({
                account: address(0),
                fee: ud60x18(0)
